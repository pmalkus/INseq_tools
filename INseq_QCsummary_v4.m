function INseq_QCsummary_v4(plot,totalReadThreshold,LRratio)
%last edit, July-25-2020
    %formerley INseq_QCsummary_v3ag (July-12-2019)

% e.g. >> INseq_QCsummary_v4(0,50,100)
    
%Acquires metrics from read tables to evaluate quality/content
%changes for _v3ag: build QC table for all sample files in current directory 
    ...
    % scatterplot option for total reads >3 filter
...
%Overview, data handling
    ...
    %Filter-C: select coordinates with totReads > user-defined threshold
...
%INPUT
    ...
    % data tables in current directory should have standard naming:
...
%PLOTS (optional, user inout; Filter-B only)
...
for i=1:size(s,2) 
    ...
    %FILTER-C: apply user-defined threshold for total reads
    clrtC = clrt((clrt(:,4) > cutTot), :);
    ...
    clrtD=clrtC(LRrat > (1/cutRat) & LRrat < cutRat,:);%final data lacks pseudocounts
    ...
    nout = num2cell(nout);
    fout = cell(4,1);
    fout{1}=sampleName; fout{2}='totalReads > 3';
    fout{3}=strcat('totalReads > ',num2str(cutTot));
    fout{4}=strcat('totalReads > ',num2str(cutTot),', L/R < ',num2str(LRratio));
    fout(:,2:6)=nout;   

    %Collect L/R bias information; scatter plot w/ totRead>3
    [lovrB,~] = getLR(plot,clrtB,sampleName,'totRead>3');
    filterName = strcat('totRead>',num2str(cutTot));
...
if plot==1
    ...
    hold off
    %save file
    figName=cell2mat(strcat(sampleName,'_filt-',filterName,'_QCv4.fig'));
    savefig(figName);
    close
