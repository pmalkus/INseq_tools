function annotate_BEI_table_v1(genbank_file,results_table)
%last edit: 04-24-2019

%Applies annotations to output from Goodman arrayed library well mapping
%INPUT
    % Genbank annotation file (with all 12 [super]contigs)
    % Arrayed match table
%OUTPUT
    % Add annotations to table and save as new file
    % Handle intergenic insertions as a separate class 
%PROCESS
    %Each supercontig put into an element of gbk structure
    %For each CDS annotation, within each supercontig, query table and 
        %collect matching indices and data
...
for i=1:length(gbkStruct)
    ...
    for j=1:length(con_idx)%for all rows of [super]contig
        c=coor(con_idx(j));%retrieve coordinate
...
table = table2array(T(2:end,1:end));
table_out = [table gene_info];
